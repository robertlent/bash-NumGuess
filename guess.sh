...
#	showRandom
	...
	do
		...
		if (($guess < $secret))
		then
			echo -n -e "\e[36m$guess\e[0m is \e[31mtoo low!\e[0m Try again! "
			read guess
		else
			echo -n -e "\e[36m$guess\e[0m is \e[31mtoo high!\e[0m Try again! "
			read guess
		fi
	done
	...
	if [[ "$count" -eq "1" ]]
	then
		echo -e "\e[32mCorrect!\e[0m \e[36m$secret\e[0m was the right number! You got it in \e[96m$count guess\e[0m! How did you know?!"
		echo
	else
		echo -e "\e[32mCorrect!\e[0m \e[36m$secret\e[0m was the right number! It took you \e[96m$count guesses\e[0m! Good job!"
		echo
	fi
...
if [[ -z "$1" ]]; then
	...
	echo
	echo "You will be offered a choice of difficulty from 1-5. The higher the difficulty, the more digits will be in the number, i.e. Difficuty 1 is a 1-digit number from 1 to 9, but difficulty 5 will be a 5 digit number from 10000 to 99999."
	echo
	echo -e "Your best score (lowest number of guesses), as well as your highest completed difficulty will be stored in a file named 'score'."
	echo
	echo -e "To play the game, type \e[32m./guess.sh play\e[0m."
	echo -e "To delete your score file, type \e[31m./guess.sh reset\e[0m."
	echo
else
	if [[ "$1" == "reset" ]]; then
